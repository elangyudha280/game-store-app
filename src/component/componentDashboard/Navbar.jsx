...
const DevideLogout = ()=>{
    ...
    return (
        <>
           <button onClick={showModalLogout} className="devide-profile  absolute  right-0 gap-3 top-[190%] w-[110px]  bg-[#1c2541] py-2 px-3 rounded-md transition-all duration-75 flex items-center shadow-[0_0_0.5em_#1c2541] font-gilroy-medium text-white hover:text-red-500 active:scale-95">
...
const Navbar = ()=>{
    ...
    return (
        <nav className="navbar relative w-full h-[62px] py-[0.9em] animate-slide px-4  bg-[#1D1725] shadow-[0_0.8em_1em_rgba(0,0,0,0.3)]">
            <div className="container-nav mx-auto max-w-7xl h-full  flex ">
                ...
                <div className="nav-item  flex-1 flex gap-2 justify-end relative items-center ">
                        <button onClick={toggleDevide} className="profile devide w-[130px] h-full gap-2 bg-[#1c2541] py-2 px-3 flex items-center rounded-full">
                            <i className={`devide arrow-devide fa-solid fa-angle-up scale-90 text-white  transition-all duration-300 ${(!state.showDevide) && 'rotate-[180deg]'}`}></i>
